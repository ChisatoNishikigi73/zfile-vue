<template>
	...
	>
		...
		<z-form-item prop="faviconUrl" label="站点 favicon">
			<el-input
				id="faviconUrl"
				v-model="data.faviconUrl"
				:prefix-icon="Tickets"
			/>
			<template #tips
				>网站在浏览器标题栏的 favicon 图标，需确保输入的地址没有 referer
				防盗链，如修改后不生效请清理浏览器缓存或再试。</template
			>
		</z-form-item>
		...
		<z-form-item prop="domain" label="后端站点域名">
			...
			<el-popover placement="right" :width="400" trigger="click">
				...
				<div class="text-sm">
					<div class="mb-2">
						检测到设置异常，实际后端域名应为
						<span class="text-red-500">{{ fixDomain.except }}</span
						>，是否需要修复？（不修正可能会影响下载、文件夹加密和文档预览功能）
					</div>
		...
		<z-form-item prop="frontDomain" label="前端站点域名">
			...
			<el-popover placement="right" :width="400" trigger="click">
				...
				<div class="text-sm">
					<div class="mb-2">
						检测到设置异常，实际前端域名应为
						<span class="text-red-500">{{ fixFrontDomain.except }}</span
						>，是否需要修复？（不修正会影响 401、403、404 页面的跳转功能）
					</div>
					<div class="flex justify-end">
						<el-button
							size="small"
		...
		<z-form-item prop="siteHomeName" label="根路径名称">
			<el-input
				id="siteHomeName"
				v-model="data.siteHomeName"
				:prefix-icon="HomeIcon"
			/>
		...
		<z-form-item prop="siteHomeLogo" label="首页 Logo 图片地址">
			<el-input
				id="siteHomeLogo"
				v-model="data.siteHomeLogo"
				:prefix-icon="PhotoIcon"
			/>
			<template #tips
				>用于设置首页上面包屑左侧
				Logo，留空为不显示，会自动居中显示，如有需要可通过自定义 css
				来修改默认样式, logo 元素的 id 为 zfile-home-logo</template
			>
		</z-form-item>
		...
		<z-form-item prop="siteHomeLogoLink" label="首页 Logo 打开链接">
			<el-input
				id="siteHomeLogoLink"
				v-model="data.siteHomeLogoLink"
				:prefix-icon="Link"
			/>
			<template #tips
				>点击 Logo 链接后打开的地址，为空则点击不进行任何跳转</template
			>
		</z-form-item>

		<z-form-item prop="siteHomeLogoTargetMode" label="首页 Logo 链接打开方式">
			<el-radio v-model="data.siteHomeLogoTargetMode" label="_blank"
				>新标签打开</el-radio
			>
			<el-radio v-model="data.siteHomeLogoTargetMode" label="_self"
				>当前页面打开</el-radio
			>
...
let dataRules = ref({
	...
	faviconUrl: [
		{
			type: 'url',
			message: '请输入正确的 favicon 地址，需以 http 或 https 开头',
		},
	],
