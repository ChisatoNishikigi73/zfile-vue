<template>
	<div class="zfile-admin-down-link-body">
		...
		>
			<template #form-title> 直/短链设置 </template>
			<template #form-sub-title> 这里可以配置直/短链相关设置 </template>

			<z-form-item label="记录下载日志">
				<el-switch v-model="data.recordDownloadLog" />
				<template #tips> 是否记录直/短链下载日志 </template>
			</z-form-item>
			...
			<z-form-item label="短链有效期">
				...
				<template #tips> 控制生成短链的有效期 </template>
			</z-form-item>
			...
			<z-form-item v-show="data.refererType !== 'no'" label="白名单">
				...
				<template #tips>
					...
					<ul>
						<li>
							限制泛域名
							<span class="text-red-400">http 协议</span> 全部子目录可访问：
							<div class="inline select-all font-bold">
								<span class="text-red-400">http://</span>*example.com/**
							</div>
						</li>
						<li>
							限制泛域名
							<span class="text-red-400">https 协议</span> 全部子目录可访问：
							<div class="inline select-all font-bold">
								<span class="text-red-400">https://</span>*example.com/**
							</div>
						</li>
						<li>
							限制泛域名
							<span class="text-red-400">全部协议</span> 全部子目录可访问：
							<div class="inline select-all font-bold">
								<span class="text-red-400">*://</span>*example.com/**
							</div>
						</li>
					</ul>
					<div v-if="data.refererType === 'white_list'">
						每行输入一个域名，需要写协议头支持 * 通配符，如白名单
						http://*zfile.vip 将只允许
						http://zfile.vip、http://www.zfile.vip、http://demo.zfile.vip
						等网站访问。
					</div>
					<div v-if="data.refererType === 'black_list'">
						每行输入一个域名，需要写协议头，支持 * 通配符，如黑名单
						http://*zfile.vip 将禁止所有如
						http://zfile.vip、http://www.zfile.vip、http://demo.zfile.vip
						等网站访问。
					</div>
					...
					<div class="mt-4">
						<div>
							可在下方输入你要测试的地址（应包含协议，路径，从浏览器地址栏复制就是了）
						</div>
						<el-input v-model="testAntVal" @input="testAntPathMatcher">
							<template #suffix>
								<CheckCircleIcon
									v-if="testAntVal && testAntResult === true"
									class="w-4 text-green-500"
								></CheckCircleIcon>
								<XCircleIcon
									v-else-if="testAntVal && testAntResult === false"
			...
			<z-form-item label="直链下载限制">
				...
				<span>秒内允许下载</span>
				<el-input-number
					v-model="data.linkDownloadLimit"
					:min="0"
					:max="9999999"
					:step="1"
					size="small"
				...
				<template #tips>
					<span
						>设置直链防止恶意下载，单 IP N 秒内只允许访问 M 次直链，如其中一个为
						0 则不做任何限制.</span
					>
					<br /><br />
					<span
						>注意：此功能对直链、短链都有效，且共享限制次数。但直链/短链跳转后的实际下载链接无法限制，因为那些链接不经过
						ZFile.</span
					>
					<br /><br />
					<span
						>注意：如果你使用了反向代理，而不是直接访问的 ZFile 端口,
						那你需要在反向代理处设置以下请求头为用户真实 IP："X-Forwarded-For",
						"X-Real-IP", "Proxy-Client-IP", "WL-Proxy-Client-IP",
						"HTTP_CLIENT_IP", "HTTP_X_FORWARDED_FOR"，不然不论谁访问 ZFile
						都只能获取到反代服务器的 IP，同服务器一般是获取到 127.0.0.1 或
						localhost, 这样此功能就无法正常使用!!! (辅助信息: 系统获取到您当前
						IP 为 <span class="text-blue-400">{{ clientIp }}</span
						>)</span
					>
				</template>
			</z-form-item>
