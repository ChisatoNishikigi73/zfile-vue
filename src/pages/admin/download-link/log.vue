	...
	<div class="zfile-admin-down-link">
		<el-card>
			<div class="flex justify-between">
				<h3 class="text-lg leading-6 font-medium text-gray-900">下载日志</h3>
				<div
					v-if="data"
			...
			<div class="mt-4">
				...
				<div class="mb-2">
					...
					<el-button
						:icon="Delete"
						type="danger"
						@click="batchDeleteLinkLogByQuery"
						>删除全部页</el-button
					>
				</div>
				...
				>
					...
					>
						<template #default="scope">
							<div v-if="scope.row.shortKey" class="space-x-2">
								<span>{{ generateShortLink(scope.row.shortKey) }}</span>
								<svg-icon
									class="ml-2 inline cursor-pointer"
									name="copy"
									@click="copyText(generateShortLink(scope.row.shortKey))"
								></svg-icon>
								<svg-icon
									class="ml-1 inline cursor-pointer text-blue-500 text-sm"
									name="target"
									@click="openLink(generateShortLink(scope.row.shortKey))"
								></svg-icon>
							</div>
							<div v-else>
								...
								<svg-icon
									class="ml-2 inline cursor-pointer"
									name="copy"
									@click="copyText(generateLink(scope.row))"
								></svg-icon>
								<svg-icon
									class="ml-1 inline cursor-pointer text-blue-500 text-sm"
									name="target"
									@click="openLink(generateLink(scope.row))"
								></svg-icon>
							</div>
						</template>
					</el-table-column>
...
const batchDeleteLinkLogByQuery = () => {
	ElMessageBox.confirm(
		`是否确认删除当前查询条件下的 ${searchParam.total} 条直链下载日志？(如数量不对，请先点击查询后再用该功能按条件删除)`,
		'提示',
		{
			type: 'warning',
		}
...
let generateShortLink = (shortKey) => {
	return common.removeDuplicateSeparator(
		`${systemConfig?.value?.domain}/s/${shortKey}`
	)
}

let generateLink = (row) => {
	return common.removeDuplicateSeparator(
		`${systemConfig?.value?.domain}/${systemConfig?.value?.directLinkPrefix}/${row.storageKey}/${row.path}`
	)
}
</script>
