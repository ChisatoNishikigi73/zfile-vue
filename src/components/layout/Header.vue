<template>
	<div class="zfile-header">
		<div
			class="zfile-header-breadcrumb box animate__animated animate__fadeIn flex flex-1"
		>
			<header-logo></header-logo>
			<breadcrumb
				class="h-12"
				:items="breadcrumbData"
				@breadcrumb-click="clickBreadcrumb"
			>
			</breadcrumb>
		</div>
...
onMounted(() => {
	nextTick(() => {
		if (storageConfigStore.globalConfig.customJs) {
			...
			Array.from(tempDivDom.getElementsByTagName('script')).forEach(
				(script) => {
