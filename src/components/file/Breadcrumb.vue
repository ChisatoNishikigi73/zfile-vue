<template>
	<nav ref="parentEl" class="flex w-full relative z-breadcrumbs-root">
		<ol ref="breadcrumbEl" role="list" class="z-breadcrumbs">
			...
			<template v-else>
				<li v-if="foldBreadcrumb.fold?.length > 0" class="z-breadcrumbs__fold">
					<div class="z-breadcrumbs__item">
						<ChevronRightIcon
							v-if="foldBreadcrumb.head.length > 0"
							class="z-breadcrumbs__icon"
						/>
						<el-dropdown
										...
										<a
											...
											:class="[
												item === props.items[props.items.length - 1]
													...
													: 'text-gray-600 hover:text-gray-700 font-semibold',
												'  max-w-md overflow-ellipsis overflow-hidden whitespace-nowrap',
											]"
										>
											{{ item?.name }}
										</a>
		...
		<ol
			...
			role="list"
			class="!flex-nowrap z-breadcrumbs invisible absolute"
		>
		...
		<a
			ref="hideBreadcrumbEllipsisEl"
			class="z-breadcrumbs__ellipsis invisible absolute"
		>
			···
		</a>
	</nav>
